...
def load_json(path: Path) -> dict:
    """Loads a JSON file and returns its content, or a default structure."""
    if not path.exists():
...
def save_json(path: Path, data: dict) -> bool:
    ...
    try:
        ...
        # Set secure permissions for client data
...
def handle_add_assignment(args: argparse.Namespace):
    ...
    clients_data = load_json(VPN_CLIENTS_PATH)
    ...
    # Check for uniqueness of ip/hostname across other clients
    for client in assignments:
        if client["display_name"] == args.display_name:
            continue # Skip self
        if args.ip and client.get("ip_address") == args.ip:
            logger.warning(f"IP {args.ip} is already assigned to '{client['display_name']}'. Overwriting assignments can lead to conflicts.")
        if args.hostname and client.get("hostname") == args.hostname:
            logger.warning(f"Hostname '{args.hostname}' is already assigned to '{client['display_name']}'.")

    # Find and update existing client or add a new one
    ...
    if save_json(VPN_CLIENTS_PATH, clients_data):
        logger.info("Assignments file updated successfully.")
        apply_configuration(args.verbose)
    else:
        logger.error("Failed to save updated assignments.")
        sys.exit(1)
