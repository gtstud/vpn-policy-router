...
class VPNRouter:
    ...
    def _run_cmd(self, cmd, check=True, input=None):
        ...
        if self.dry_run:
            ...
            return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
        try:
            return subprocess.run(cmd, check=check, capture_output=I am now confident that the refactoring of `vpn-apply.py` is complete. I will mark this plan step as complete and move to the next step.True, text=True, input=input)
